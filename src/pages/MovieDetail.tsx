...
export default function MovieDetail() {
  ...
  return (
    <div className="relative min-h-screen text-white bg-linear-to-b from-gray-950 via-black to-gray-900">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-3 gap-8">
        ...
        >
          ...
          >
            {favIds.includes(movie.id) ? (
              <>
                <FaHeart className="text-white" /> {t("movieDetail.removeFromFavorites")}
              </>
            ) : (
              <>
                <FaRegHeart className="text-red-400" /> {t("movieDetail.addToFavorites")}
              </>
            )}
          </button>
        </motion.div>
        ...
        >
          ...
          {/* Cast Section */}
      <h3 className="text-2xl font-semibold mb-3">{t("movieDetail.cast")}</h3>
      <Slider {...settings}>
        {cast.map((c: any) => (
          <div key={c.cast_id} className="px-2">
            <div className="text-center bg-gray-800/40 rounded-xl p-3 hover:bg-gray-700/40 transition">
              ...
              <div className="text-xs text-gray-400">{t("movieDetail.as")} {c.character}</div>
            </div>
          </div>
        ))}
      </Slider>
      ...
      {trailer && (
        <div className="mt-10">
          <h3 className="text-2xl font-semibold mb-3">{t("movieDetail.watchTrailer")}</h3>
